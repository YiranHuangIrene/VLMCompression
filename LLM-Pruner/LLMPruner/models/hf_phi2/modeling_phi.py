...
class PhiAttention(nn.Module):
    ...
    def _init_rope_prune(self, head_dim_new):  #FIXME: ugly fix for the change of hidden size
            if self.config.rope_scaling is None:
                self.rotary_emb = PhiRotaryEmbedding(
                    int(self.partial_rotary_factor * head_dim_new),
                    max_position_embeddings=self.max_position_embeddings,
                    base=self.rope_theta,
                )
            else:
    ...
    def forward(
        ...
        query_states = query_states.view(bsz, q_len, self.num_heads, query_states.shape[-1] // self.num_heads).transpose(1, 2)
        key_states = key_states.view(bsz, q_len, self.num_heads, key_states.shape[-1]// self.num_heads).transpose(1, 2)
        value_states = value_states.view(bsz, q_len, self.num_heads, value_states.shape[-1] // self.num_heads).transpose(1, 2)
        # self._init_rope_prune(query_states.shape[-1])
        ...
        if attn_output.size() != (bsz, self.num_heads, q_len, query_states.shape[-1]):
            raise ValueError(
                f"`attn_output` should be of size {(bsz, self.num_heads, q_len, query_states.shape[-1])}, but is"
                f" {attn_output.size()}"
            )
