...
class DistillationTrainer(LLaVATrainer):
    ...
    def get_teacher(self, teacher_args, data_args, compute_dtype, attn_implementation):
        ...
        if teacher_args.vision_tower is not None:
            if 'mpt' in teacher_args.model_name_or_path:
                config = transformers.AutoConfig.from_pretrained(teacher_args.model_name_or_path, trust_remote_code=True)
        ...
        if teacher_args.vision_tower is not None:
            ...
            model.config.mm_use_im_start_end = data_args.mm_use_im_start_end = teacher_args.mm_use_im_start_end
            model.config.mm_projector_lr = self.args.mm_projector_lr
            self.args.use_im_start_end = teacher_args.mm_use_im_start_end
            model.config.mm_use_im_patch_token = teacher_args.mm_use_im_patch_token
            model.initialize_vision_tokenizer(teacher_args, tokenizer=self.tokenizer)
    ...
    @staticmethod
    def get_log_probs(logits, ids, mask=None, inf_mask=None, model_parallel=False):
        if model_parallel:
            logprobs = -transformers.mpu.parallel_logprobs(logits, ids)
            if inf_mask is not None:
                gathered_inf_mask = transformers.mpu.parallel_gather(inf_mask, -1, ids.unsqueeze(-1)).squeeze(-1)
    ...
    def compute_logits_and_log_probs(self, logits, labels, inf_mask=None, return_logprobs=True):
        logits = logits / self.args.dist_temperature
        ...
        if return_logprobs:
            logprobs = self.get_log_probs(logits, labels, mask, inf_mask, model_parallel=self.is_model_parallel)
