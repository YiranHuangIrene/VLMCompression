...
class TrainingArguments(transformers.TrainingArguments):
    ...
    output_dir: str = field(
        default="/shared-local/aoq609/VLMCompression/VLM/llava/checkpoint/distill-test"
    )
    ...
    wandb_group: str = field(default=None)
    ...
    teacher_name_or_path: Optional[str] = field(default='liuhaotian/llava-v1.5-7b')
...
def load_non_lora_trainable_params(model, lora_path):
    weight_path = os.path.join(lora_path, 'non_lora_trainables.bin')
...
def train(attn_implementation=None):
    ...
    wandb.init(project="vlmcompression",
           settings={
            ...
            group=f'{training_args.wandb_group}',
           )
    ...
    # engine, _, _, _ = deepspeed.initialize(args=training_args, model=model, model_parameters=model.parameters())
